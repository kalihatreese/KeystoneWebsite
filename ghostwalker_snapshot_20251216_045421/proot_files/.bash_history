...
git commit -m "Full Keystone narrative + Veritas greeting"
...
pwd
git add .
git commit -m "Fix Trinity server, static UI, clean port binding"
git config --global user.name "kalihatreese"
git config --global user.email "kalihatreese@gmail.com"
cd ~/KeystoneCreatorSuite/trinity
git config --global user.name "kalihatreese"
git config --global user.email "kalihatreese@gmail.com"
git init
git add .
git commit -m "Initial commit for Trinity website"
git init
git add .
git commit -m "Initial commit for Trinity website"
git push -u origin main
git remote add origin git@github.com:kalihatreese/trinity.git
git push -u origin main
cd ..
vercel
npm run build
mv ~/KeystoneCreatorSuite/trinity/public ~/KeystoneCreatorSuite/public
cd ~/KeystoneCreatorSuite
npm run build
cd ~/KeystoneCreatorSuite
vercel --prod
rm -f ~/Downloads/chat_exports/merged_chat.json
rm -f ~/Downloads/chat_exports/full_merged_chat.json
rm -f ~/Downloads/chat_exports/full_chat_memory_for_chatgpt.json
rm -f ~/Downloads/chat_exports/memory_flat.json
rm -f ~/ShadowX/shadowx_ready.json
rm -rf ~/ChatGPT_memory
conversations.json
group_chats.json
...
mkdir -p ~/ChatGPT_memory ~/ShadowX
# Recursively find all JSON files on SD card and process them
find /storage/external-1/ShadowX -type f -name '*.json' | while read f; do   jq --stream '
    select(length == 2) |
    .[1] |
    select(type=="object" and has("timestamp") and has("speaker") and has("content"))
  ' "$f" >> ~/ChatGPT_memory/full_memory.json; done
# Copy the flattened file for ShadowX
cp ~/ChatGPT_memory/full_memory.json ~/ShadowX/shadowx_ready.json
# Create output folders
mkdir -p ~/ChatGPT_memory ~/ShadowX
# Recursively find all JSON files on SD card and process them
find /storage/external-1/ShadowX -type f -name '*.json' | while read f; do   jq --stream '
...
cd downloads 
ls
# Create working folders on internal storage
mkdir -p ~/ChatGPT_memory ~/ShadowX
# Process all JSON files from SD card and append to one flattened file
find /storage/external-1/ShadowX -type f -name '*.json' | while read f; do   jq --stream '
    select(length == 2) |
    .[1] |
    select(type=="object" and has("timestamp") and has("speaker") and has("content"))
  ' "$f" >> ~/ChatGPT_memory/full_memory.json; done
# Copy flattened JSON for ShadowX
cp ~/ChatGPT_memory/full_memory.json ~/ShadowX/shadowx_ready.json
# Optional: Split into 100MB chunks for easier loading
split -b 100M ~/ShadowX/shadowx_ready.json ~/ShadowX/chunk_
# Create working folders on internal storage
mkdir -p ~/ChatGPT_memory ~/ShadowX
# Process all JSON files from SD card and append to one flattened file
find /storage/external-1/ShadowX -type f -name '*.json' | while read f; do   jq --stream '
...
pm2 status trinity  # if trinity is the Keystone process
# Check PM2 processes
pm2 list
# Check if Node/Next.js server is listening
netstat -tulpn | grep LISTEN
pm2 list                 # check status
pm2 restart trinity       # restart AI core
pm2 logs trinity          # watch for errors
cd ~/KeystoneCreatorSuite/trinity
pm2 start index.js --name trinity --update-env
pm2 list
pm2 logs trinity
mkdir -p ~/ChatGPT_memory ~/ShadowX
cp ~/ChatGPT_memory/full_memory.json ~/ShadowX/shadowx_ready.json
split -b 100M ~/ShadowX/shadowx_ready.json ~/ShadowX/chunk_
llmkdir -p ~/ChatGPT_memory ~/ShadowX
ps aux | grep node
kill -9 14128
ps aux | grep node
